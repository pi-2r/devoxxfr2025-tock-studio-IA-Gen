...
#
"""
Index a CSV file for severeal bots (line format: 'title'|'source'|'text') into a vector database.

Usage:
  index_documents_for_all_accounts.py --input-csv=<path> \
                     --account-csv=<act_csv> \
                     --output-account-csv=<out_act_csv> \
                     ...
                     [--env-file=<env>] [-v]
  index_documents_for_all_accounts.py (-h | --help)
  index_documents_for_all_accounts.py --version

Options:
  -h --help                           Show this help message.
  --version                           Show the version.
  --account-csv=<path>                Input lab account CSV file.
  --output-account-csv=<path>                Input lab account CSV file.
...
if __name__ == '__main__':
    ...
    csv_account_csv_path = args['--account-csv']
    output_csv_account_csv_path = args['--output-account-csv']
    ...
    # Load account CSV file using panda
    accounts_df = pd.read_csv(csv_account_csv_path, delimiter=',', quotechar='"', header=0, dtype=str)

    for index, row in accounts_df.iterrows():
        namespace = row['org']
        bot_id = "devoxx2025"
        print(f"==> Handling namespace: : {namespace} ...")

        namespace = namespace
        bot_id = bot_id
        details = index_documents()

        # Proper way to assign a value to a cell in the DataFrame
        accounts_df.at[index, 'indexing_session_uuid'] = details.indexing_session_uuid

        logging.info(details.format_indexing_details())

    # Save the modified DataFrame to a new CSV
    accounts_df.to_csv(output_csv_account_csv_path, index=False)
